...
from components.execution.task_manager import TaskManager
...
class BackgroundTask:
    ...
    def __init__(self, task_id: str, task_name: str = "Benchmark"):
    ...
    def _init_task_file(self):
        ...
        self.task_manager.create_task(self.task_id, self.task_name)
        # Also keep backward compatible task file
        self.update_status({
    ...
    def update_status(self, status_dict: dict) -> None:
        """Update task status file with new data (thread-safe)."""
        self._ensure_dir()
        with BackgroundTask._file_lock:
            with open(self.TASK_FILE, "w") as f:
                json.dump(status_dict, f, indent=2)
    
    def get_status(self) -> dict:
        """Get current task status from file (thread-safe)."""
    ...
    def set_running(self) -> None:
        ...
        self.task_manager.set_task_running(self.task_id)
    ...
    def set_completed(self, results_file: str | None = None, error: str | None = None) -> None:
        ...
        if error:
            ...
            self.task_manager.set_task_failed(self.task_id, error)
        else:
            ...
            self.task_manager.set_task_completed(self.task_id, results_file)
    ...
    def should_stop(self) -> bool:
        ...
        return task and task.status == "stopped" if task else False
    ...
    def update_progress(self, current_step: int, total_steps: int, step_name: str = "") -> None:
        ...
        if self.should_stop():
            self._has_stop_signal = True
            self.log("Stop signal received, attempting graceful shutdown...")
            return
        ...
        self.update_status(status)
        self.task_manager.update_task_progress(self.task_id, progress_pct, step_name, total_steps)
...
def run_background_task(func, task_id: str, task_name: str = "Benchmark", *args, **kwargs) -> str:
    """
    ...
    """
    task = BackgroundTask(task_id, task_name)
    ...
    def wrapper():
        try:
            ...
            # If task wasn't marked as stopped mid-execution
            if task.get_status().get("status") != "stopped":
                pass  # func should have called set_completed
